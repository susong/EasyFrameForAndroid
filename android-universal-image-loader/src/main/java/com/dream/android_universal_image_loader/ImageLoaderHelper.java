package com.dream.android_universal_image_loader;
...
public class ImageLoaderHelper {
    ...
    public DisplayImageOptions getDisplayOptionsPic() {
        return new DisplayImageOptions.Builder()
                .showImageForEmptyUri(R.drawable.auil_picture_loading)//设置图片Uri为空或是错误的时候显示的图片
                .showImageOnLoading(R.drawable.auil_picture_loading)//设置图片在下载期间显示的图片
                .showImageOnFail(R.drawable.auil_picture_loadfailed)//设置图片加载/解码过程中错误时候显示的图片
...
//            .displayer(new FadeInBitmapDisplayer(100))//是否图片加载好后渐入的动画时间
    ...
    public DisplayImageOptions getDisplayOptions() {
        return new DisplayImageOptions.Builder()
                .showImageOnLoading(R.color.auil_default_image_background)
                .showImageForEmptyUri(R.color.auil_default_image_background)
                .showImageOnFail(R.color.auil_default_image_background)
                .cacheInMemory(true)
                .cacheOnDisk(true)
    ...
    public DisplayImageOptions getDisplayOptions(int round) {
        return new DisplayImageOptions.Builder()
                ...
                .imageScaleType(ImageScaleType.EXACTLY)
                .considerExifParams(true)
                .displayer(new RoundedBitmapDisplayer(dip2px(mContext, round)))
                .build();
    }
...
//        builder.memoryCacheSize(2 * 1024 * 1024);//内存缓存大小
        ...
        if (displayImageOptions == null) {
            builder.defaultDisplayImageOptions(getDisplayOptionsPic());
        } else {
